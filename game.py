...
def get_scavenge_result(state):
    ...
    if roll < 0.1:
        ...
        base_fuel = state.rng.randint(0, 2)
        base_food = state.rng.randint(1, 3)
...
def cmd_raid(state, energy_spent, rival_id):
    ...
    if player_wins:
        ...
        # Для простоты: если дронов больше чем у соперника — подчиняем
        if state.drones >= rival["drones"]:
...
def process_rivals(state):
    ...
    for rival_id, rival in state.rivals.items():
        ...
        # Создание дронов (если уровень >= 2)
        if rival["level"] >= 2 and rival["scrap"] >= 5 and rival["fuel"] >= 1:
            rival["scrap"] -= 5
            rival["fuel"] -= 1
            rival["drones"] += 1
